...
| Registre d'images | Docker Hub (`brice99/cicd-sonarqub`) |
...
| `VPS_HOST` | Adresse IP ou nom de domaine du VPS | Fourni par l'hébergeur du VPS |
...
| `SONAR_PROJECT_KEY` | Clé du projet SonarCloud | `bricengueti_cicd_sonarqub` |
...
|---|---|---|---|---|
| `IMAGE_NAME` | Nom du dépôt Docker Hub — **identique dans les 3 environnements** | `brice99/cicd-sonarqub` | `brice99/cicd-sonarqub` | `brice99/cicd-sonarqub` |
...
| `CONTAINER_NAME` | Nom du conteneur Docker sur le VPS | `cicd-sonarqub-test` | `cicd-sonarqub-preprod` | `cicd-sonarqub-prod` |
...
Résultats disponibles sur `https://sonarcloud.io/project/overview?id=bricengueti_cicd_sonarqub` — un sélecteur de branche permet de consulter séparément les résultats de `test`, `preprod`, `prod`, ou d'une Pull Request.
...
Environnements déployés (à adapter selon l'IP réelle du VPS) :

- Test : `http://<IP_DU_VPS>:3000`
- Preprod : `http://<IP_DU_VPS>:3001`
- Prod : `http://<IP_DU_VPS>:3002`
...
|---|---|---|---|
| 1 | Push normal sur `test` | `git push origin test` | Les 6 stages s'exécutent et se terminent en succès ; le conteneur `cicd-sonarqub-test` est `(healthy)` sur le VPS |
...
|---|---|---|
| Test | `http://<IP_DU_VPS>:3000` | Badge bleu **TEST** |
| Preprod | `http://<IP_DU_VPS>:3001` | Badge orange **PREPROD** |
| Prod | `http://<IP_DU_VPS>:3002` | Badge rouge **PROD** |
